    ...
    <div className="space-y-5">
        ...
        <div className="flex gap-2">
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-[#e8e3d8] transition-all duration-150"
            ...
            {showPresets && (
              <div className="absolute right-0 top-full mt-1 w-72 rounded-xl shadow-2xl z-10 py-1"
                ...
                {presets.map((preset, idx) => (
                  ...
                  >
                    <div className="text-sm font-semibold text-[#e8e3d8]">{preset.name}</div>
                    <div className="text-xs text-[#6b6455] mt-0.5">
          ...
          <button
            ...
            style={showAdd
              ? { background: "#12100b", color: "#e8e3d8", border: "1px solid rgba(255,255,255,0.06)" }
              : { background: "#ffb224", color: "#0b0a08", border: "none" }
            }
          >
      ...
      {showAdd && (
        <form onSubmit={handleCreate} className="rounded-xl p-5 animate-fade-in"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-[11px] font-bold text-[#8a8271] mb-1 uppercase tracking-wider">Name</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#8a8271] mb-1 uppercase tracking-wider">Command</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#8a8271] mb-1 uppercase tracking-wider">Cron</label>
              ...
              {form.cron_expression && (
                <p className="text-xs text-[#ffb224] mt-1 opacity-70">{describeCron(form.cron_expression)}</p>
              )}
            </div>
          </div>
          <div className="flex flex-wrap gap-1.5 mt-3">
            {CRON_PATTERNS.map((p) => (
              <button
                ...
                style={{
                  background: form.cron_expression === p.value ? "#ffb224" : "#12100b",
                  color: form.cron_expression === p.value ? "#0b0a08" : "#8a8271",
                  border: `1px solid ${form.cron_expression === p.value ? "#ffb224" : "rgba(255,255,255,0.06)"}`,
                }}
              >
                {p.label}
              </button>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            {createMutation.isError && <span className="text-xs text-[#d96b5c]">Failed to create schedule</span>}
          </div>
        </form>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 rounded-xl" style={{ background: "#0b0a08", border: "1px solid rgba(255,255,255,0.06)" }}>
          <Clock className="h-8 w-8 text-[#12100b] mx-auto mb-2" />
          <p className="text-sm text-[#8a8271]">No scheduled commands yet</p>
          <p className="text-xs text-[#6b6455] mt-1">Add a schedule or apply a preset</p>
        </div>
      ) : (
            ...
            <tbody>
              {schedules.map((cmd) => (
                <tr key={cmd.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                  {editingId === cmd.id ? (
                    <>
                      <td className="px-4 py-2"><input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="w-full rounded-md px-2 py-1 text-xs text-[#e8e3d8] focus:outline-none" style={inputStyle} /></td>
                      <td className="px-4 py-2"><input value={editForm.command} onChange={(e) => setEditForm({ ...editForm, command: e.target.value })} className="w-full rounded-md px-2 py-1 text-xs text-[#e8e3d8] focus:outline-none" style={{ ...inputStyle, fontFamily: "var(--font-mono)" }} /></td>
                      <td className="px-4 py-2"><input value={editForm.cron_expression} onChange={(e) => setEditForm({ ...editForm, cron_expression: e.target.value })} className="w-full rounded-md px-2 py-1 text-xs text-[#e8e3d8] focus:outline-none" style={{ ...inputStyle, fontFamily: "var(--font-mono)" }} /></td>
                      <td colSpan={3} />
                      <td className="px-4 py-2 text-right">
                        <button onClick={() => saveEdit(cmd.id)} disabled={updateMutation.isPending} className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-bold text-[#0b0a08] mr-1" style={{ background: "#ffb224" }}>
                          <Check className="h-3 w-3" /> Save
                        </button>
                        <button onClick={() => setEditingId(null)} className="inline-flex items-center rounded-md px-2 py-1 text-xs text-[#e8e3d8]" style={{ background: "#12100b" }}>
                          <X className="h-3 w-3" />
                        </button>
                      </td>
                    </>
                    ...
                    <>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          <span className={`h-1.5 w-1.5 rounded-full ${cmd.is_active ? "bg-[#ffb224] status-online" : "bg-[#6b6455]"}`} />
                          <span className="text-sm text-[#e8e3d8] font-medium">{cmd.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-xs text-[#8a8271] max-w-[200px] truncate" style={{ fontFamily: "var(--font-mono)" }}>{cmd.command}</td>
                      <td className="px-4 py-2.5">
                        <div className="text-xs text-[#8a8271]" style={{ fontFamily: "var(--font-mono)" }}>{cmd.cron_expression}</div>
                        <div className="text-[11px] text-[#6b6455]">{describeCron(cmd.cron_expression)}</div>
                      </td>
                      <td className="px-4 py-2.5 text-xs text-[#6b6455]">{timeAgo(cmd.last_run)}</td>
                      <td className="px-4 py-2.5 text-xs text-[#6b6455]">{cmd.is_active ? timeUntil(cmd.next_run) : "\u2014"}</td>
                      <td className="px-4 py-2.5 text-xs text-[#6b6455]" style={{ fontFamily: "var(--font-mono)" }}>{cmd.run_count}</td>
      ...
      {schedules.some((s) => s.last_result) && (
        <details className="rounded-xl" style={{ background: "#0b0a08", border: "1px solid rgba(255,255,255,0.06)" }}>
          <summary className="px-4 py-2.5 text-xs font-bold text-[#6b6455] cursor-pointer hover:text-[#8a8271] uppercase tracking-wider">
          ...
          <div className="px-4 py-3 space-y-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            {schedules.filter((s) => s.last_result).map((s) => (
              <div key={s.id}>
                <span className="text-xs font-bold text-[#e8e3d8]">{s.name}:</span>
                <pre className="text-xs text-[#6b6455] mt-0.5 whitespace-pre-wrap break-all" style={{ fontFamily: "var(--font-mono)" }}>
                  {s.last_result}
                </pre>
              </div>
            ))}
